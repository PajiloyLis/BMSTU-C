...
#define THIRD_MINIMUM_NOT_FOUND 14
...
int main(void)
{
    ...
    int *arr = NULL;
    node_t list = NULL;
    if (scanf("%lld", &n_buf) != 1 || n_buf < 3)
    {
        printf("Third minimum could not be found\n");
        rc = ARRAY_SIZE_ERROR;
    }
    else
    {
    ...
    if (rc == EXIT_SUCCESS)
    {
        ...
        node_t to_del = rc == EXIT_SUCCESS ? find_third_min(list) : NULL;
        if (!to_del)
            rc = THIRD_MINIMUM_NOT_FOUND;
        list = rc == EXIT_SUCCESS ? delete_node_by_pointer(list, to_del) : list;
        if (rc == EXIT_SUCCESS)
            list_print(list);
        if (list)
            list_free(list);
